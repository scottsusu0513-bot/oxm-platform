...
const ERP_STATUS_COLORS: Record<string, CaseStatusColor> = {
  new: "blue",
  needs_triage: "cyan",
  diagnosis: "cyan",
  solution_design: "violet",
  proposal: "violet",
  in_progress: "amber",
  pilot_adjustment: "amber",
  acceptance: "teal",
  completed: "emerald",
  deferred: "orange",
  no_interest: "rose",
...
const ERP_STATUS_DEFS: CaseStatusDef[] = ERP_CASE_STATUSES.map(key => ({
  key,
  label: ERP_STATUS_LABELS[key],
  color: ERP_STATUS_COLORS[key] ?? "slate",
}));
...
function statusColor(s: string): string {
  const COLORS: Record<string, string> = {
    ...
    needs_triage: "bg-cyan-100 text-cyan-700",
    diagnosis: "bg-sky-100 text-sky-700",
    solution_design: "bg-teal-100 text-teal-700",
    proposal: "bg-violet-100 text-violet-700",
    in_progress: "bg-amber-100 text-amber-700",
    pilot_adjustment: "bg-yellow-100 text-yellow-700",
    acceptance: "bg-indigo-100 text-indigo-700",
...
function NotesEditor({ caseId, initialNotes, onMutated }: { caseId: number; initialNotes: string | null; onMutated: () => void }) {
  ...
  const mutation = trpc.erpConsultant.updateCaseNotes.useMutation({
...
function AssignConsultantControl({ caseId, currentConsultantId, onMutated }: { caseId: number; currentConsultantId: number | null; onMutated: () => void }) {
  const { data: consultants } = trpc.erpConsultant.adminListConsultants.useQuery();
  const mutation = trpc.erpConsultant.adminAssignConsultant.useMutation({
        ...
        <ul className="mt-2 space-y-1.5 border-l-2 border-border pl-3">
          {entries.map((h, i) => (
            <li key={i} className="text-muted-foreground">
              <span className="font-medium text-foreground">{ERP_STATUS_LABELS[h.status as ErpCaseStatus] ?? h.status}</span>
...
function StatusAdvanceControls({ item, onMutated }: { item: any; onMutated: () => void }) {
  const [pending, setPending] = useState<ErpCaseStatus | null>(null);
  const [reason, setReason] = useState("");
  const statusMutation = trpc.erpConsultant.updateCaseStatus.useMutation({
    onSuccess: () => { toast.success("狀態已更新"); setPending(null); setReason(""); onMutated(); },
    onError: (err) => toast.error(err.message || "更新失敗"),
  });
  const nextOptions = ERP_STATUS_TRANSITIONS[item.status as ErpCaseStatus] ?? [];
  if (nextOptions.length === 0) return null;

  function handleClick(opt: ErpCaseStatus) {
    if (ERP_EXCEPTION_STATUSES.includes(opt)) {
      setPending(opt);
      setReason("");
    } else {
      ...
      <div className="flex flex-wrap gap-2">
        {nextOptions.map(opt => (
          <Button key={opt} size="sm" variant={pending === opt ? "default" : "outline"} disabled={statusMutation.isPending} onClick={() => handleClick(opt)}>
            {ERP_STATUS_LABELS[opt]}<ArrowRight className="w-3.5 h-3.5 ml-1.5" />
          </Button>
        ))}
      </div>
      {pending && (
        <div className="space-y-2 rounded-lg border border-amber-200 dark:border-amber-900/40 bg-amber-50 dark:bg-amber-950/20 p-3">
          <p className="text-xs text-amber-800 dark:text-amber-300">進入「{ERP_STATUS_LABELS[pending]}」必須填寫原因：</p>
    ...
    <Card>
      <CardContent className="p-5 space-y-3">
        ...
        <div className="text-sm space-y-1">
          <p><span className="text-muted-foreground">需求類型：</span>{erpNeedTypeLabel(item.needType)}</p>
...
function UnassignedCaseCard({ item, onClaimed }: { item: any; onClaimed: () => void }) {
  const claimMutation = trpc.erpConsultant.claimCase.useMutation({
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container py-24 flex flex-col items-center text-center space-y-6 max-w-sm mx-auto">
        <div className="w-16 h-16 rounded-full bg-orange-50 flex items-center justify-center"><LogIn className="w-8 h-8 text-orange-600" /></div>
        <div className="space-y-2">
          <h2 className="text-xl font-bold">請先登入</h2>
          <p className="text-muted-foreground text-sm leading-relaxed">登入後，若您是啟用中的 ERP 顧問或管理員，即可查看案件看板。</p>
        </div>
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container py-24 flex flex-col items-center text-center space-y-4 max-w-sm mx-auto">
        ...
        <p className="text-muted-foreground text-sm">{message || "您目前不是啟用中的 ERP 顧問，也不是管理員。"}</p>
...
export default function ErpConsultantCases() {
  const { user, loading: authLoading } = useAuth();
  const utils = trpc.useUtils();
  const [selectedStatus, setSelectedStatus] = useState("all");

  const casesQuery = trpc.erpConsultant.myCases.useQuery({}, { enabled: !!user, retry: false });
  const isAdmin = !!user?.isAdmin;
  const unassignedQuery = trpc.erpConsultant.unassignedCases.useQuery(undefined, {
    enabled: !!user && !isAdmin && !casesQuery.error, retry: false,
  });

  function handleMutated() {
    utils.erpConsultant.myCases.invalidate();
    utils.erpConsultant.unassignedCases.invalidate();
  }
  ...
  const statusDefs = isAdmin ? ERP_STATUS_DEFS : ERP_STATUS_DEFS.filter(s => s.key !== "unassigned");
  const items = selectedStatus === "all" ? baseItems : baseItems.filter(c => c.status === selectedStatus);

  return (
    <div className="min-h-screen bg-background">
      <Helmet><title>ERP 顧問案件看板｜OXM</title></Helmet>
      <Navbar />
      <FloatingBackButton fallbackHref="/" />
      <div className="container py-8 md:py-12 max-w-3xl mx-auto space-y-8">
        <h1 className="text-xl md:text-2xl font-bold">ERP 與產線優化案件看板</h1>
            ...
            {unassignedItems.length === 0 ? (
              ...
              <div className="space-y-4">
                {unassignedItems.map((item: any) => (
                  <UnassignedCaseCard key={item.id} item={item} onClaimed={handleMutated} />
                ))}
              </div>
            )}
          </div>
