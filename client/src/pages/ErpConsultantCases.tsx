...
import { ERP_STATUS_LABELS, ERP_STATUS_TRANSITIONS, erpNeedTypeLabel } from "@shared/erpOptimization";
...
function CaseCard({ item, isAdmin, onMutated }: { item: any; isAdmin: boolean; onMutated: () => void }) {
  ...
  const nextOptions = ERP_STATUS_TRANSITIONS[item.status] ?? [];
        ...
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div className="space-y-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <Badge className={statusColor(item.status)}>{ERP_STATUS_LABELS[item.status as keyof typeof ERP_STATUS_LABELS] ?? item.status}</Badge>
            </div>
