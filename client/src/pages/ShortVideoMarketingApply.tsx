...
const AI_PREFILLABLE_FIELD_KEYS = ["servicesWanted", "isUnsure", "primaryGoal", "platforms", "noPlatformYet"];
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container py-24 flex flex-col items-center text-center space-y-6 max-w-md mx-auto">
        ...
        <h1 className="text-2xl font-bold">已送出短影音與品牌內容免費初步諮詢申請</h1>
        <p className="text-muted-foreground leading-relaxed">
          顧問將儘速查收案件並主動與您聯繫，進行初次諮詢。
        </p>
        <div className="flex flex-col sm:flex-row gap-3">
          <Link href="/short-video-marketing">
          ...
          <Link href="/">
            <Button className="bg-gradient-to-r from-orange-500 to-purple-600 hover:opacity-90 text-white border-0">
              返回首頁
            </Button>
          </Link>
        </div>
...
export default function ShortVideoMarketingApply() {
  ...
  const [servicesWanted, setServicesWanted] = useState<ShortVideoServiceKey[]>([]);
  ...
  const { data: progressData, isLoading: progressLoading } = trpc.shortVideoCenter.myApplicationProgress.useQuery(undefined, {
    enabled: !!user,
    staleTime: 30_000,
  });
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<FormValues>({ defaultValues: { agreeTerms: false, primaryGoal: "", additionalNotes: "" } });
  ...
  // 「不確定」與五項明確服務互斥：勾選「不確定」時清空 servicesWanted；
  // 反過來勾選任一明確服務時自動取消「不確定」。
  const toggleService = (key: ShortVideoServiceKey) => {
    setIsUnsure(false);
    setServicesWanted(prev => prev.includes(key) ? prev.filter(k => k !== key) : [...prev, key]);
  };
  ...
  // 「尚未經營」與其他平台互斥，理由同上。
  const togglePlatform = (key: ShortVideoPlatformKey) => {
    setNoPlatformYet(false);
    setPlatforms(prev => prev.includes(key) ? prev.filter(k => k !== key) : [...prev, key]);
  };
  const toggleNoPlatform = (checked: boolean) => {
    setNoPlatformYet(checked);
    if (checked) setPlatforms([]);
  };

  const servicesValid = isUnsure || servicesWanted.length > 0;
  const platformsValid = noPlatformYet || platforms.length > 0;

  const onSubmit = (data: FormValues) => {
    if (!selectedFactory || !data.primaryGoal) return;
  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-background">
        <Helmet><title>短影音與品牌內容免費初步諮詢｜OXM</title><meta name="robots" content="noindex, nofollow, noarchive, nosnippet" /></Helmet>
        <Navbar />
        <GateView
          icon={<LogIn className="w-8 h-8 text-orange-600" />}
          title="請先登入"
          description="登入後，即可申請短影音與品牌內容免費初步諮詢服務。"
          actions={
            <>
              <Button
                className="w-full bg-gradient-to-r from-orange-500 to-purple-600 hover:opacity-90 text-white border-0"
              ...
              <Link href="/short-video-marketing">
                <Button variant="outline" className="w-full">返回短影音與品牌內容行銷</Button>
              </Link>
            </>
          }
        />
    ...
    return (
      <div className="min-h-screen bg-background">
        ...
        <GateView
          ...
          title="請先上架您的工廠"
          description="請先在 OXM 平台上架工廠，通過審核後即可申請短影音與品牌內容免費初步諮詢。"
          actions={
            <>
              <Link href="/register-factory">
                <Button className="w-full bg-gradient-to-r from-orange-500 to-purple-600 hover:opacity-90 text-white border-0">
                  前往上架工廠
                </Button>
              </Link>
              <Link href="/short-video-marketing">
  ...
  if (eligibleFactories.length === 0 && anyFactoryStatus !== null) {
    ...
    const description = isPending
      ? "您的工廠資料正在審核中，通過後即可申請短影音與品牌內容免費初步諮詢。"
      : isRejected
  ...
  if (selectedFactory && selectedOpenCase) {
    const statusLabel = SHORT_VIDEO_STATUS_LABELS[selectedOpenCase.status as keyof typeof SHORT_VIDEO_STATUS_LABELS] ?? selectedOpenCase.status;
    return (
      <div className="min-h-screen bg-background">
        <Helmet><title>短影音與品牌內容免費初步諮詢｜OXM</title><meta name="robots" content="noindex, nofollow, noarchive, nosnippet" /></Helmet>
        <Navbar />
            ...
            {eligibleFactories.map(f => {
              ...
              return (
                <button
                  ...
                  className="w-full text-left rounded-xl border border-border p-4 hover:border-orange-400 hover:bg-orange-50/50 transition-colors"
                >
                  <div className="font-semibold">{f.name}</div>
                  <div className="text-xs text-muted-foreground truncate">{f.address}</div>
                  {openCase && (
                    <div className="text-xs text-orange-600 mt-1">
                      目前狀態：{SHORT_VIDEO_STATUS_LABELS[openCase.status as keyof typeof SHORT_VIDEO_STATUS_LABELS] ?? openCase.status}
                    </div>
                  )}
                </button>
              );
      ...
      <div className="container py-10 md:py-16 max-w-2xl mx-auto space-y-8">
        <button
          ...
          onClick={() => {
            ...
            else navigate("/short-video-marketing");
          }}
          className="flex items-center gap-1.5 text-muted-foreground hover:text-foreground transition-colors text-sm"
          aria-label="返回短影音與品牌內容行銷"
        >
        ...
        <div className="space-y-1">
          <h1 className="text-2xl md:text-3xl font-bold">短影音與品牌內容免費初步諮詢</h1>
          <p className="text-muted-foreground">請填寫聯絡資料與需求，合作顧問將主動與您聯繫安排初次諮詢。</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
          ...
          <fieldset className="space-y-4 rounded-xl border border-border p-6">
            <legend className="px-1 text-sm font-semibold text-muted-foreground">想了解的服務 <span className="text-destructive">*</span>（可複選）</legend>
            <div className="grid sm:grid-cols-2 gap-2.5">
              {SHORT_VIDEO_SERVICES.map(s => (
                <label key={s.key} className="flex items-center gap-2 text-sm rounded-lg border border-border p-3 cursor-pointer hover:bg-muted/40">
                  <Checkbox checked={servicesWanted.includes(s.key)} onCheckedChange={() => toggleService(s.key)} disabled={isUnsure} />
                  {s.label}
                </label>
              ))}
            </div>
            ...
            {!servicesValid && <p className="text-xs text-destructive">請選擇至少一項服務，或勾選「不確定」</p>}
          ...
          <fieldset className="space-y-3 rounded-xl border border-border p-6">
            <legend className="px-1 text-sm font-semibold text-muted-foreground">主要目標 <span className="text-destructive">*</span>（單選）</legend>
            <RadioGroup value={primaryGoal} onValueChange={(v) => setValue("primaryGoal", v as ShortVideoGoalKey)}>
              <div className="space-y-2">
                {SHORT_VIDEO_GOALS.map(g => (
                  <label key={g.key} className="flex items-center gap-2 text-sm rounded-lg border border-border p-3 cursor-pointer hover:bg-muted/40">
                    <RadioGroupItem value={g.key} id={`goal-${g.key}`} />
                    {g.label}
                  </label>
                ))}
              </div>
            </RadioGroup>
            {!primaryGoal && <p className="text-xs text-destructive">請選擇本次主要目標</p>}
            <AiPrefillFieldHint confirmedFields={aiHandoff.confirmedFields} fieldKey="primaryGoal" />
          </fieldset>

          <fieldset className="space-y-4 rounded-xl border border-border p-6">
            <legend className="px-1 text-sm font-semibold text-muted-foreground">目前經營平台（可複選）</legend>
            <div className="grid sm:grid-cols-2 gap-2.5">
              {SHORT_VIDEO_PLATFORMS.map(p => (
                <label key={p.key} className="flex items-center gap-2 text-sm rounded-lg border border-border p-3 cursor-pointer hover:bg-muted/40">
                  <Checkbox checked={platforms.includes(p.key)} onCheckedChange={() => togglePlatform(p.key)} disabled={noPlatformYet} />
                  {p.label}
                </label>
              ))}
            </div>
            <label className="flex items-center gap-2 text-sm rounded-lg border border-dashed border-border p-3 cursor-pointer hover:bg-muted/40">
              <Checkbox checked={noPlatformYet} onCheckedChange={(v) => toggleNoPlatform(v === true)} />
              尚未經營
            </label>
            {!platformsValid && <p className="text-xs text-destructive">請選擇目前經營的平台，或勾選「尚未經營」</p>}
            <AiPrefillFieldHint confirmedFields={aiHandoff.confirmedFields} fieldKey="platforms" />
            <AiPrefillFieldHint confirmedFields={aiHandoff.confirmedFields} fieldKey="noPlatformYet" />
          </fieldset>
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-2">
            ...
            <Button
              type="submit"
              disabled={!agreeTerms || !servicesValid || !platformsValid || !primaryGoal || applyMutation.isPending}
          ...
          {(!agreeTerms || !servicesValid || !platformsValid || !primaryGoal) && (
