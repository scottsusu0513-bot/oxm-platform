...
function statusColor(s: string): string {
  const COLORS: Record<string, string> = {
    ...
    pre_production: "bg-sky-100 text-sky-700",
    script_review: "bg-teal-100 text-teal-700",
    in_progress: "bg-amber-100 text-amber-700",
    draft_review: "bg-yellow-100 text-yellow-700",
    delivered: "bg-lime-100 text-lime-700",
    ongoing_operation: "bg-indigo-100 text-indigo-700",
        ...
        <ul className="mt-2 space-y-1.5 border-l-2 border-border pl-3">
          {entries.map((h, i) => (
            <li key={i} className="text-muted-foreground">
              <span className="font-medium text-foreground">{SHORT_VIDEO_STATUS_LABELS[h.status as ShortVideoCaseStatus] ?? h.status}</span>
...
function StatusAdvanceControls({ item, onMutated }: { item: any; onMutated: () => void }) {
  const [pending, setPending] = useState<ShortVideoCaseStatus | null>(null);
  ...
  function handleClick(opt: ShortVideoCaseStatus) {
    if (SHORT_VIDEO_EXCEPTION_STATUSES.includes(opt)) {
      setPending(opt);
      setReason("");
    } else {
    ...
    <div className="space-y-2 pt-1">
      ...
      {pending && (
        <div className="space-y-2 rounded-lg border border-amber-200 dark:border-amber-900/40 bg-amber-50 dark:bg-amber-950/20 p-3">
          <p className="text-xs text-amber-800 dark:text-amber-300">進入「{SHORT_VIDEO_STATUS_LABELS[pending]}」必須填寫原因：</p>
        ...
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div className="space-y-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <Badge className={statusColor(item.status)}>{SHORT_VIDEO_STATUS_LABELS[item.status as ShortVideoCaseStatus] ?? item.status}</Badge>
              {item.aiAssessment && <AiOriginBadge />}
            </div>
            <p className="text-xs text-muted-foreground truncate">{item.companyAddressSnapshot}</p>
          </div>
          {item.aiAssessment && <AiAssessmentDialogTrigger serviceKey="short_video" assessment={item.aiAssessment} />}
        </div>
...
function UnassignedCaseCard({ item, onClaimed }: { item: any; onClaimed: () => void }) {
  const claimMutation = trpc.shortVideoConsultant.claimCase.useMutation({
    ...
    <Card className="border-dashed">
      <CardContent className="p-5 space-y-3">
        <div className="flex items-center gap-2 flex-wrap">
          <Building2 className="w-4 h-4 text-muted-foreground shrink-0" />
          <span className="font-semibold truncate">{item.companyNameSnapshot}</span>
          <Badge className={statusColor("unassigned")}>{SHORT_VIDEO_STATUS_LABELS.unassigned}</Badge>
          {item.aiAssessment && <AiOriginBadge />}
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1 text-sm">
          <div className="flex items-center gap-1.5"><UserIcon className="w-3.5 h-3.5 text-muted-foreground" />聯絡人：{item.contactName}</div>
        ...
        <p className="text-sm"><span className="text-muted-foreground">想了解的服務：</span>{services.join("、")}</p>
        {item.aiAssessment && <AiAssessmentDialogTrigger serviceKey="short_video" assessment={item.aiAssessment} />}
...
export default function ShortVideoConsultantCases() {
  ...
  const unassignedQuery = trpc.shortVideoConsultant.unassignedCases.useQuery(undefined, {
    enabled: !!user && !isAdmin && !casesQuery.error, retry: false,
  });

  function handleMutated() {
    utils.shortVideoConsultant.myCases.invalidate();
    utils.shortVideoConsultant.unassignedCases.invalidate();
  }
