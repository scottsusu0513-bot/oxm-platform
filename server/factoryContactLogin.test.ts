...
// 1. 工廠頁的 handleChat 未登入時要開啟登入選擇彈窗，不能直接呼叫 performLogin()
// 2. 登入選擇彈窗本身（LoginDialog）不能在掛載時就自動觸發任何登入 provider

const FACTORY_DETAIL_PATH = path.resolve(
  import.meta.dirname,
  "..",
  "client",
  "src",
...
describe("FactoryDetail.tsx: 未登入點擊聯繫商家", () => {
  const source = fs.readFileSync(FACTORY_DETAIL_PATH, "utf-8");
  ...
  it("handleChat 未登入時開啟登入選擇彈窗，而不是直接呼叫 performLogin()", () => {
