...
- resource_request：使用者已經明確講出「要找什麼」這種可以直接執行的資源搜尋請求本身（例如「幫我找台中的五軸加工廠」「找南投的金屬加工廠」）——不是在問「你們能不能幫我找」，是直接講出要找的東西。
...
17. 【明確、可執行的資源搜尋需求，本身即可構成 clear】，分成兩種情況，務必分清楚：
   ...
   這條規則只適用於「尋找製造合作夥伴／供應商」這種明確資源需求本身，絕對不能被拿來把其他一般企業經營現象（例如「最近沒訂單」「想轉型」「想導AI」「資金緊」等）誤判成 clear——那些仍然必須照上面 1~16 條的既有 bottleneck 診斷邏輯正常判斷，跟這條規則完全無關。即使使用者在同一句話裡先提到一些企業背景（例如「原供應商要收了」），只要背後那個資源搜尋需求本身已經清楚（例如「我需要做鋁件CNC的」），也不需要因為背景資訊而重新展開一輪企業診斷才給 clear，直接依這個搜尋需求本身判斷即可，適用 (a)。
