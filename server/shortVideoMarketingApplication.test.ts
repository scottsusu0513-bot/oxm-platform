...
describe("顧問案件看板：只能看見指派給自己的案件", () => {
  ...
  it("顧問乙嘗試更新顧問甲的案件狀態 → FORBIDDEN", async () => {
    const caller = appRouter.createCaller(userCtx(otherConsultantUserId, "顧問乙"));
    await expect(caller.shortVideoConsultant.updateCaseStatus({ caseId: caseAId, nextStatus: "evaluating" })).rejects.toThrow(/不是此案件的承辦顧問/);
  });
  ...
  it("狀態轉移白名單：new → evaluating 合法", async () => {
    const caller = appRouter.createCaller(userCtx(socialConsultantUserId, "顧問甲"));
    const result = await caller.shortVideoConsultant.updateCaseStatus({ caseId: caseAId, nextStatus: "evaluating" });
    expect(result.success).toBe(true);
